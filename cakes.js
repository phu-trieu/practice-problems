...
const cakes = (recipe, available) => {
  ...
  for (const ingredient in recipe) {
    const maximum = Math.floor(available[ingredient] / recipe[ingredient]);
    if (!available[ingredient]) return 0;
    if (lowest === undefined || maximum < lowest) lowest = maximum;
    // if (maximum < lowest) lowest = maximum
  }
  return lowest;
}

console.log(cakes({ flour: 500, sugar: 200, eggs: 1, milk: 200, butter: 5 }, { flour: 4000, sugar: 1200, eggs: 5, milk: 200 }));
